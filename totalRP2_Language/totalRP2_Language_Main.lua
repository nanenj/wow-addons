...
function TRP2_InitDialectList()
	...
	for i=1,GetNumLanguages() do
		...
		if not LanguesTab[langue] then
			TRP2_AddLangage(langue,100,true,true);
		end
	end
...
function TRP2_HookSendChatMessage()
	function TRP2_SendChatMessage( msg, chatType, languageID, channel )
		...
		if string.find(TRP2_DIALBASETAB[TRP2_enRace], TRP2_GetInfo(TRP2_Joueur,"SelectDial", select(1, GetDefaultLanguage("player")))) then
...
function TRP2_TransformMessage(msg,ID,Comp,bColor)
	...
	if string.find(msg,"|H") or string.find(TRP2_DIALBASETAB[TRP2_enRace],ID) or string.sub(msg,1,1) == "|" then 
		return msg;
	end
	...
	if not string.find(TRP2_DIALBASETAB[TRP2_enRace],ID) then
