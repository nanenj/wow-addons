...
function TRP2_nomComplet(nom,bNoTitre)
...
function TRP2_EstDansLeReg(nom)
	return TRP2_Module_Registre[TRP2_Royaume][nom] ~= nil;
end
...
function TRP2_MSP_Request(nom)
	...
	if not TRP2_EstIgnore(nom) and msptrp.char[ nom ].supported ~= false and string.sub(msptrp.char[nom].field['VA'],1,8) ~= "TotalRP2" then
		...
		--TRP2_debug("Requested : "..tostring(requested));
	end
end
...
function TRP2_MSP_HistoryUpdate(histoireTab,noUpdate)
	msptrp.my['HI'] = TRP2_CTS(TRP2_GetWithDefaut(histoireTab,"HistoireTexte",""));
	msptrp.my['HI'] = TRP2_EraseTags(msptrp.my['HI']);
	TRP2_MSP_Update(noUpdate);
end

function TRP2_MSP_DescriUpdate(physiqueTab,noUpdate)
	msptrp.my['DE'] = TRP2_CTS(TRP2_GetWithDefaut(physiqueTab,"PhysiqueTexte",""));
	msptrp.my['DE'] = TRP2_EraseTags(msptrp.my['DE']);
	TRP2_MSP_Update(noUpdate);
end
...
function TRP2_MSP_ActuUpdate(actuTab,noUpdate)
	...
	-- Currently
	msptrp.my['CU'] = TRP2_CTS(TRP2_GetWithDefaut(actuTab,"ActuTexte",""));
	TRP2_MSP_Update(noUpdate);
end

function TRP2_MSP_RegistreUpdate(registreTab,noUpdate)
	-- Name
	msptrp.my['NA'] = TRP2_CTS(TRP2_nomComplet(TRP2_Joueur));
	-- Title
	msptrp.my['NT'] = TRP2_CTS(TRP2_GetWithDefaut(registreTab,"TitreComplet",""));
	-- Race
