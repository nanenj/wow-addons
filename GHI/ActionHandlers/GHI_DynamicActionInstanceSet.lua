...
function GHI_DynamicActionInstanceSet(item, firstPortID, firstPortInfo)
	...
	class.Deserialize = function(t)      --print("set deserialized from data",class)
		...
		for i, instanceData in pairs(t.instances) do
			instances[i] = GHI_DynamicActionInstance(nil, instanceData.guid);
			-- statistics
	...
	class.Execute = function(portGuid, stack, instanceIndex, isUpdateAction, feedbackFunctions)   --  print("execute on set",class)
		...
		if instance then
			instance.Execute(instancePortGuid, stack, instanceIndex, isUpdateAction, feedbackFunctions);
		end
	end
