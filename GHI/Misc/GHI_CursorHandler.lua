...
function GHI_CursorHandler()
	...
	class.SetCursor = function(cursorType, cursorDetail, onClearFunc, onOverlayClickFunc, ...)
		...
		if cursorType == "ITEM" then
			...
			local type2,stackGuid,slotID, stack, amount, itemGuid  = ...;
			if type2 == "GHI_ITEM" or type2 == "GHI_ITEM_REF" then
				actionBar.ShowAll(itemGuid,cursorDetail,function() class.ClearCursor(); end);
			end
		else
			cursorItemIcon:Hide();
		end
