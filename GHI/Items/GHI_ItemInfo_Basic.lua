...
function GHI_ItemInfo_Basic(info)
	...
	class.GenerateStack = function(amount)
		local stackInfo = {};
	...
	class.GetTooltipLines = function()
		...
		local timeSinceLastCast = time() - class.GetLastCastTime();
		if (timeSinceLastCast < class.GetCooldown()) then
			table.insert(lines, {
				order = 60,
				text = string.format("%s %s", loc.CD_LEFT, misc.GHI_GetTimeString(class.GetCooldown() - timeSinceLastCast));
				r = 1,
				g = 1,
				b = 1,
			});
