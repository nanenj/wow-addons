...
function GHI_ItemInfo_Advanced(info)
	...
	class.GetDependingItems = function(stack)
		return dynamicActionSet.GetDependingItems(stack);
	end
	...
	class.UseItem = function(stack)
		...
		if dynamicActionSet then
			dynamicActionSet.Execute("onclick", stack, stack.GetFirstItemInstanceByOrder(class.GetStackOrder()));
		end
