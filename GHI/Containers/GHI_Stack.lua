...
function GHI_Stack(parentContainer, info1, info2, tempItemInfo)
	...
	local Initialize = function()
			...
			if #(info1) > 0 then
				...
				for i = 1, #(info1) do
					if info1[i].amount > 0 then
						table.insert(itemInstances, GHI_ItemInstance(info1[i]));
					end
				end
			else
				itemInstances = { GHI_ItemInstance(info1) };
			end
		elseif type(info1) == "string" and type(info2) == "number" then
			...
			local instance = GHI_ItemInstance(info2);
	...
	class.GetAttribute = function(attributeName, itemInstanceIndex)
		...
		if not(item.GetItemComplexity) then error("Incomplete item info",item.GetType and item.GetType()); end
		if not(item.GetItemComplexity() == "advanced") then
			return
		end
		...
		if instance then       --print("attribute found",attributeName)
			return instance.attributeValues[attributeName];
		end
		--print("attribute found (default)",attributeName)
		--print("item",class.GetItemInfo())

		local attribute = item.GetAttribute(attributeName);
		if attribute then
			return attribute.GetDefaultValue();
		end   --print("No attribute named",attributeName)
	end
	...
	class.SetAttribute = function(attributeName, value, itemInstanceIndex)
		...
		if instance then
			if attribute and attribute.ValidateValue(value) then
				instance.attributeValues[attributeName] = value;
			end
		end
		event.TriggerEvent("GHI_CONTAINER_UPDATE", parentContainer.GetGUID())
	end
