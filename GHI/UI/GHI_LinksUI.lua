...
function GHI_LinksUIHandler()
	...
	class.GenerateLink = function(prefix, text, ID, color)
		GHCheck("GHI_LinksUI.GenerateLink", { "String", "String", "String", "Table" }, { prefix, text, ID, color })
		return string.format("|cFF%s%s%s|Hgh_%s:%s:0:0:0:0:0:0:0|h[%s]|h|r", string.format("%.2x", color.r * 255), string.format("%.2x", color.g * 255), string.format("%.2x", color.b * 255), prefix, ID, text);
	end
	...
	GetCleanTextAndLinksFromText = function(text, chatType)
		local clean = string.gsub(text, "|cFF(%x%x)(%x%x)(%x%x)|Hgh_(.-):(.-):0:0:0:0:0:0:0|h%[(.-)%]|h|r", function(r, g, b, prefix, ID, name)
	...
	IdentifyAndSendLinkInfo = function(sender, chatType, linkName)
		for _, link in pairs(sendLinks) do
			...
			if link.name == linkName and (link.chatType == chatType or chatType == "any") then
				comm.Send("ALERT", sender, "LinkConfirm", link.name, link.prefix, link.ID, link.chatType, link.color);
	...
	RecieveLinkConfirm = function(sender, name, prefix, ID, chatType, color)
		local link = class.GenerateLink(prefix, name, ID, color);
