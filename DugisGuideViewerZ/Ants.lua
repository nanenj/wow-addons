...
function Ants:Initialize()
	...
	function Ants:UpdateAntTrail(elapsed)
		...
		if DGV.DugisArrow.waypoints then
			...
			for index, objective in ipairs(DGV.DugisArrow.waypoints) do
				...
				if x1 and x1~=x2 and y1~=y2 then
					...
					if DugisGuideViewer:UserSetting(DGV_SHOWANTS) and not DugisGuideViewer.WrongInstanceFloor and len > 0.0001 then
						...
						-- World Map
						if WorldMapFrame:IsVisible() and not DugisGuideViewer.carboniteloaded then
							while p < 1 do
				...
				if DGV.DugisArrow:DidPlayerReachWaypoint() and not DugisGuideViewer:UserSetting(DGV_CARBONITEARROW) then
					DGV.DugisArrow:SetNextWaypoint()
				end
			end
		else
			-- Clear Ant Trail
