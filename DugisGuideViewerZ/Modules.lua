...
local function PlaceUtilityStubs(name)
	...
	if evalName("Guides") then
		...
		function DGV:ShowLargeWindow()
			DGV:CreateSettingsTree(DugisMainframe)
			DugisGuideViewer.SettingsTree.frame:SetPoint("TOPLEFT", DugisMainframe, "TOPLEFT", "20", "-20")
			DugisGuideViewer.SettingsTree.frame:SetPoint("BOTTOMRIGHT", DugisMainframe, "BOTTOMRIGHT", "-50", "20")
			DugisMainframe:SetHeight(377)
			DugisMainframe:Show()
			Dugis:Hide()
...
StaticPopupDialogs["DUGIS_RELOAD_PROMPT"] = {
	text = L["Re-enabling these Dugi Guides features will require the UI to reload.  Do this now?"],
	button1 = L["Yes"],
	button2 = L["No"],
	OnAccept = function()
		ReloadUI()
...
local function ShouldLoad(module)
	...
	return (DugisGuideViewer.db.char.settings.EssentialsMode<1 or module.essential) and DugisGuideViewer.GuideOn
end

local function InitModule(module)
	if not module.initialized and not module.loaded then
		if not module.Initialize then ShowReloadUi();return false end --module deleted
		module:Initialize()
