...
function MapOverlays:Initialize()
	...
	--For Orcs, Hyjal is called Hyjal_terrain1.  I dunno, ask Blizzard.
	...
	local function OverrideMapOverlays()
		...
		--DebugPrint("Debug OverrideMapOverlays: mapName="..tostring(mapName))
		if not mapName then
			return
		end
		local mapDirectory
		...
		local numOverlayTextures = GetNumMapOverlays()
		--DebugPrint("Debug OverrideMapOverlays: numOverlayTextures="..numOverlayTextures);
		...
		--DebugPrint("#overlayTextures: "..numOverlayTextures);
		for texName, texData in pairs(overlayData) do
			...
			--DebugPrint("numTexturesTall: "..numTexturesTall);
			--DebugPrint("neededTextures: "..neededTextures);
			if neededTextures > numOverlayTextures  then
				for j = numOverlayTextures  + 1, neededTextures do
					local texture = WorldMapDetailFrame:CreateTexture(format("WorldMapOverlay%d", j), "ARTWORK")
	...
	function MOD:InitializeMapOverlays()
		DebugPrint("Debug: InitializeMapOverlays");
		db = LibStub("AceDB-3.0"):New("MapOverlays", defaults)
	...
	function MapOverlays:Load()
		UpdateWorldMapFrame = function()
			if not MOD.CoordsFrame then
				DebugPrint("Debug UpdateWorldMapFrame: WorldMapFrame="..tostring(WorldMapFrame))
				...
				MOD.CoordsFrame:SetScript("OnUpdate", function()
					...
					then
						...
						if not x or not y then
							MOD.CoordsFrame.Player:SetText("|cfff0c502Player:|r ---")
						else
							MOD.CoordsFrame.Player:SetFormattedText("|cfff0c502Player:|r %s", formatCoords:format(x*100, y*100))
						end

						if WorldMapButton:GetLeft() then --prevents error on early load
							local cX, cY = MOD:GetCurrentCursorPosition(WorldMapButton)
							if not cX or not cY then
								MOD.CoordsFrame.Cursor:SetText("|cfff0c502Cursor:|r ---")
							else
								MOD.CoordsFrame.Cursor:SetFormattedText("|cfff0c502Cursor:|r %s", formatCoords:format(cX*100, cY*100))
							end
						end
					else
					...
					if DugisGuideViewer.carboniteloaded
						or DugisGuideViewer.tomtomloaded
			...
			if WORLDMAP_SETTINGS.size == WORLDMAP_WINDOWED_SIZE then
				MOD.CoordsFrame.Player:SetPoint("BOTTOMLEFT", WorldMapDetailFrame, "BOTTOMLEFT", 8, -19)
				MOD.CoordsFrame.Cursor:SetPoint("LEFT", MOD.CoordsFrame.Player, "RIGHT", 4, 0)
			else
				MOD.CoordsFrame.Player:SetPoint("RIGHT", WorldMapPositioningGuide, "BOTTOM", -15, 15)
				MOD.CoordsFrame.Cursor:SetPoint("LEFT", WorldMapPositioningGuide, "BOTTOM", 15, 15)
			end
			OverrideMapOverlays()
		end
